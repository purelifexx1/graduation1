...
namespace LAN_project
{
    class ObjVolume : Volume
    {
        ...
        public static List<ObjVolume> LoadFromFile_mul_mtl(string filename)
        {
            List<ObjVolume> objects = new List<ObjVolume>();
            try
            {
                using (StreamReader reader = new StreamReader(new FileStream("robot/"+filename, FileMode.Open, FileAccess.Read)))
                {
                    objects = LoadFromString_mul_mtl(reader.ReadToEnd());
                }
            }
            catch (FileNotFoundException e)
            {
        ...
        public static List<ObjVolume> LoadFromString_mul_mtl(string obj)
        {
            List<String> lines = new List<string>(obj.Split('\n'));
            string object_material = string.Empty;
            foreach (string line in lines)
            {
                if(line.StartsWith("mtllib "))
                {
                    object_material = line.Substring(7, line.IndexOf('\r') - 7);
                    break;
                }
            }
            List<ObjVolume> object_part = new List<ObjVolume>();
            ObjVolume tag;
            
            Dictionary<String, Material> materials = new Dictionary<string, Material>();
            foreach (var mat in Material.LoadFromFile("material/"+object_material))
            {
            ...
            foreach (String line in lines)
            {
                if (line.StartsWith("v ")) // Vertex definition
                {
                    ...
                    if (temp.Trim().Count((char c) => c == ' ') == 2) // Check if there's enough elements for a vertex
                    {
                        ...
                        colors.Add(new Vector3((float)Math.Sin(vec.Y), (float)Math.Sin(vec.Y), (float)Math.Sin(vec.Y)));
                ...
                else if (line == "")
                {
                    ...
                    foreach (var face in faces)
                    {
                        ...
                        tag.faces.Add(new Tuple<FaceVertex, FaceVertex, FaceVertex>(v1, v2, v3));
                    }
                    tag.Material = materials[object_material];
                    object_part.Add(tag);
                }
                else if (line.StartsWith("usemtl  "))
                {
                    ...
                    if (mesh_start == false)
                    {
                        object_material = line.Substring(8, 2);
                        mesh_start = true;
                    }
                    else
                    {
                        ...
                        tag.Material = materials[object_material];
                        object_part.Add(tag);
                        faces = new List<Tuple<TempVertex, TempVertex, TempVertex>>();
                        object_material = line.Substring(8, 2);
                    }
                }
